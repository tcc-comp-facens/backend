...
VALID_FONTES = {"siops", "datasus"}


@st.composite
def any_etl_record(draw):
    """Generate either a SIOPS-normalized record or a DataSUS record."""
    is_siops = draw(st.booleans())
    if is_siops:
        row = draw(valid_siops_row())
        return normalize_row(row, _COL_MAP)
    else:
        return draw(valid_datasus_record())


@settings(max_examples=100)
@given(record=any_etl_record())
def test_p3_any_etl_node_has_valid_fonte(record):
    """
    **Validates: Requirements 1.3, 2.6**

    For any node generated by the ETL pipeline (SIOPS or DataSUS), the `fonte`
    field must be present and contain one of the valid values: "siops" or "datasus".
    """
    assert record is not None, "ETL record must not be None for valid input"
    assert "fonte" in record, f"'fonte' field missing from ETL record: {record}"
    assert record["fonte"] in VALID_FONTES, (
        f"fonte must be one of {VALID_FONTES}, got {record['fonte']!r}"
    )
